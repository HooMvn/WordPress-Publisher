...
- Error Workflow جداگانه با `errorTrigger`، ثبت در `wordpress-publisher_errors`، هشدار مدیر.
...
2. `job_id` یکتا در `owordpress-publisher_publish_queue` (جلوگیری از Job تکراری برای یک پلتفرم).
...
فایل [`./docker-compose.queue-mode.example.yml`](./docker-compose.queue-mode.example.yml) عیناً از پروژه قبلی گرفته شده. **هیچ تغییری لازم نبود** چون هر دو پروژه از همان الگوی زیرساختی (n8n Main + Worker + Redis + Postgres، Queue Mode) استفاده می‌کنند. تنها تغییر، کامنت توضیحی بالای فایل است.
...
به [`TEST_REPORT.md`](./TEST_REPORT.md) مراجعه کنید.

**شفاف‌سازی مهم:** هیچ تماس واقعی به API واقعی Telegram/Bale/Eitaa/Rubika/X/LinkedIn یا یک نمونه واقعی WordPress برقرار نشد (چون Credential/Endpoint واقعی در اختیار من نیست). تست‌ها در دو سطح انجام شدند:

1. **تست منطقی واقعی (اجرا شد)** — کد دقیقاً همان `jsCode` داخل هر Code Node از فایل‌های `.json` نهایی استخراج و با Node.js روی ورودی‌های شبیه‌سازی‌شده (Mock) واقعاً اجرا شد؛ نتیجه هر تست PASS/FAIL واقعی است، نه ادعایی.
2. **تست ساختاری** — همه ۴ Workflow با JSON Parser Validate شدند و صحت تمام Connectionها (وجود Node مقصد، عدم نام تکراری) به‌صورت برنامه‌نویسی‌شده بررسی شد.

آنچه تست **نشد** و صادقانه اعلام می‌شود: فراخوانی واقعی n8n Runtime (چون n8n نصب‌شده در این محیط نیست)، فراخوانی واقعی هر ۶ API پلتفرم، و رفتار واقعی Data Table n8n (چون بدون یک نمونه n8n واقعی قابل تست نیست).
